...
(async () => {
  ...
  // del físico viejo se guardan los 173 renglones, pero solo cuentan los 34 que
  // siguen en la hoja: la hoja se estrechó a lo que de verdad se lleva
  check('dice cuántos productos se contaron', home.includes('34 productos contados'));
  ...
  check('no muestra la comparación con merma', await page.isHidden('#inv-comparacion'));
  ...
  check('dice cuántos van de cuántos', (await page.textContent('#fis-resumen')).includes('34 de 39'));

  // ---------- 3) las categorías de la hoja, plegadas ----------
  const cats = await page.$$eval('.inv-grupo', els => els.map(e => e.dataset.g));
  check('están los 3 grupos de la hoja', cats.length === 3, cats.length);
  check('bebidas primero, que es casi todo lo que se cuenta',
    cats.join('|') === 'Bebidas|Pollo y papas|Otros', cats);
  check('arrancan cerradas: 39 renglones de golpe no se pueden recorrer',
  ...
  check('abiertas, están los 39 renglones',
    (await page.evaluate(() => document.querySelectorAll('.fis-row').length)) === 39);

  // ---------- 4) los valores reales de la hoja ----------
  const val = async (id, k) => page.inputValue('.fis-i[data-i="' + id + '"][data-k="' + k + '"]');
  check('Refresco Uva 1L: 13 bultos', (await val('refresco_uva_1l', 'b')) === '13');
  ...
  check('Papas cestas: 387,72 kg', (await val('papas_cestas', 'u')) === '387,72');
  ...
  check('las dos columnas NO se suman entre sí',
    (await val('agua_minalba_600ml', 'b')) === '8' && (await val('agua_minalba_600ml', 'u')) === '19');
  check('lo que no se contó queda vacío',
    (await val('refresco_7up_1l', 'b')) === '' && (await val('refresco_7up_1l', 'u')) === '');
  check('la unidad de cada renglón se ve',
    (await page.textContent('.fis-row:has(input[data-i="papas_cestas"]) label:last-child em')) === 'kg');

  // ---------- 5) cerrado: no se puede tocar ----------
  check('estando cerrado, los campos se bloquean',
    await page.isDisabled('.fis-i[data-i="refresco_uva_1l"][data-k="b"]'));
  ...
  await page.fill('#fis-buscar', 'te lipton limon');
  await page.waitForTimeout(250);
  const f2 = await page.$$eval('.fis-row .fis-nom', els => els.map(e => e.textContent));
  check('busca sin acentos', f2.some(f => f.includes('Té Lipton limón')));
  ...
  check('va por categorías', wa.includes('*BEBIDAS*') && wa.includes('*POLLO Y PAPAS*'));
  ...
  check('no lista lo que no se contó', !wa.includes('Refresco 7UP 1L'));
  check('cierra con el total', wa.includes('34 productos contados'));
  ...
  await page.waitForTimeout(500);
  check('el nuevo arranca vacío', (await page.textContent('#fis-resumen')).includes('0 de 39'));
  await page.fill('.fis-i[data-i="refresco_uva_1l"][data-k="b"]', '40');
  await page.dispatchEvent('.fis-i[data-i="refresco_uva_1l"][data-k="b"]', 'change');
  await page.fill('.fis-obs[data-i="refresco_uva_1l"]', 'llegó ayer');
  await page.dispatchEvent('.fis-obs[data-i="refresco_uva_1l"]', 'change');
  await page.waitForTimeout(250);
  check('lo escrito se guarda', await page.evaluate(() =>
    currentInv.fisico.refresco_uva_1l.b === '40' && currentInv.fisico.refresco_uva_1l.obs === 'llegó ayer'));
  check('y el contador sube', (await page.textContent('#fis-resumen')).includes('1 de 39'));
  check('no se mezcla con el inventario de agosto', await page.evaluate(() =>
    db.inventarios.find(i => i.id === 'xfisico3108').fisico.refresco_uva_1l.b === '13'));

  // borrar lo escrito lo quita
  await page.fill('.fis-i[data-i="refresco_uva_1l"][data-k="b"]', '');
  await page.dispatchEvent('.fis-i[data-i="refresco_uva_1l"][data-k="b"]', 'change');
  await page.fill('.fis-obs[data-i="refresco_uva_1l"]', '');
  await page.dispatchEvent('.fis-obs[data-i="refresco_uva_1l"]', 'change');
  await page.waitForTimeout(250);
  check('vaciarlo lo quita del conteo', (await page.textContent('#fis-resumen')).includes('0 de 39'));

  // ---------- 9) sobrevive a recargar ----------
  await page.fill('.fis-i[data-i="refresco_pepsi_2l"][data-k="b"]', '22');
  await page.dispatchEvent('.fis-i[data-i="refresco_pepsi_2l"][data-k="b"]', 'change');
  await page.waitForTimeout(250);
  await page.reload();
  await page.waitForTimeout(500);
  await page.click('#home-tabs button[data-t="inventario"]');
