...
(async () => {
  ...
  check('dice cuántos van de cuántos', (await page.textContent('#fis-resumen')).includes('173 de 245'));
  ...
  check('arrancan cerradas: 245 renglones de golpe no se pueden recorrer',
  ...
  check('abiertas, están los 245 renglones',
    (await page.evaluate(() => document.querySelectorAll('.fis-row').length)) === 245);
  ...
  await page.waitForTimeout(500);
  check('el nuevo arranca vacío', (await page.textContent('#fis-resumen')).includes('0 de 245'));
  ...
  check('y el contador sube', (await page.textContent('#fis-resumen')).includes('1 de 245'));
  ...
  await page.dispatchEvent('.fis-obs[data-i="aceite_vatel_pro"]', 'change');
  await page.waitForTimeout(250);
  check('vaciarlo lo quita del conteo', (await page.textContent('#fis-resumen')).includes('0 de 245'));
